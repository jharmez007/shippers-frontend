...
import { shippingLinesMainLinks, ShippingLinesAccountLinks } from "../../constants/dummy";
import { images } from "../../constants";

const ShippingLinesSidebar = () => {
      ...
      <>
        ...
        >
          ...
          <nav className="space-y-4">
            {shippingLinesMainLinks.map(({ name, icon: Icon, path }) => (
              <NavLink
                key={name}
                to={path}
                onClick={() => setIsOpen(false)}
          ...
          <div>
            ...
            <div className="space-y-4">
              {ShippingLinesAccountLinks.map(({ name, icon: Icon, path }) => (
                <NavLink
                  key={name}
                  to={path}
                  onClick={() => setIsOpen(false)}
...
export default ShippingLinesSidebar
