...
import { nscStreamsMainLinks, nscStreamsAccountLinks } from "../../constants/dummy";
import { images } from "../../constants";

const NscStreamsSidebar = () => {
      ...
      <>
        ...
        >
          ...
          <nav className="space-y-4">
            {nscStreamsMainLinks.map(({ name, icon: Icon, path }) => (
              <NavLink
                key={name}
                to={path}
                onClick={() => setIsOpen(false)}
          ...
          <div>
            ...
            <div className="space-y-4">
              {nscStreamsAccountLinks.map(({ name, icon: Icon, path }) => (
                <NavLink
                  key={name}
                  to={path}
                  onClick={() => setIsOpen(false)}
...
export default NscStreamsSidebar
