...
const NscStreamsKpiTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (result?.data?.data) {
        ...
        toast.error(result.message || 'Failed to fetch KPI data');
      }
      setLoading(false);
    };
    fetchData();
  ...
  const handleConfirm = async (id) => {
    ...
    const result = await confirmKPI(payload);
            ...
            <table className="w-full text-sm text-left bg-white">
              ...
              {paginated.map((item) => {
                ...
                return (
                  ...
                  >
                    <td className="p-4">{item.cargo_type}</td>
...
export default NscStreamsKpiTable;
