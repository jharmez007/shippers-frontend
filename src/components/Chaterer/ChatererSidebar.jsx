...
import { chatererMainLinks, chatererAccountLinks } from "../../constants/dummy";
import { images } from "../../constants";


const ChatererSidebar = () => {
      ...
      <>
        ...
        >
          ...
          <nav className="space-y-4">
            {chatererMainLinks.map(({ name, icon: Icon, path }) => (
              <NavLink
                key={name}
                to={path}
                onClick={() => setIsOpen(false)}
          ...
          <div>
            ...
            <div className="space-y-4">
              {chatererAccountLinks.map(({ name, icon: Icon, path }) => (
                <NavLink
                  key={name}
                  to={path}
                  onClick={() => setIsOpen(false)}
...
export default ChatererSidebar
