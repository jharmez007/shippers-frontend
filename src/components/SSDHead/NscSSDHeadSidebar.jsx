...
import { ssdHeadMainLinks, ssdHeadAccountLinks } from "../../constants/dummy";
import { images } from "../../constants";

const NscSSDHeadSidebar = () => {
      ...
      <>
        ...
        >
          ...
          <nav className="space-y-4">
            {ssdHeadMainLinks.map(({ name, icon: Icon, path }) => (
              <NavLink
                key={name}
                to={path}
                onClick={() => setIsOpen(false)}
          ...
          <div>
            ...
            <div className="space-y-4">
              {ssdHeadAccountLinks.map(({ name, icon: Icon, path }) => (
                <NavLink
                  key={name}
                  to={path}
                  onClick={() => setIsOpen(false)}
...
export default NscSSDHeadSidebar
